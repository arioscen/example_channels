...
var domain_name = "";
var http_domain_name = domain_name ? "http://"+domain_name : "";
var ws_domain_name = domain_name ? domain_name : window.location.host;
//var session_id;
//var socket;
//var user_name;
//var user_picture;
...
var liveChatId = readCookie('live_chat')
// listen event
$( document ).ready(function() {
    // 取得對方的名稱、頭像
    getProfile('bot');
    // 取得使用者的 session_id
    $.get(http_domain_name + "/get_session_id?live_chat=" + liveChatId, function(response) {
        session_id = response;
        if (!readCookie('live_chat')){
            setCookie('live_chat',response,365);
        };
        // 取得使用者的名稱、頭像
        getProfile(session_id)
        // Websocket 連線
        socket = new WebSocket('ws://' + ws_domain_name + '/users/?session_id='+session_id);
        ...
        socket.onmessage = function (message) {
            ...
            // 一般訊息的處理
            if (data["opponent_id"]){
                var data_name = eval("name_"+data["opponent_id"]);
                var data_picture = eval('picture_'+data["opponent_id"]);
                updateLog(data_name, data_picture, data["msg"], data["pic"]);
            ...
            // 歷史訊息的處理
                ...
                for (var m in data){
                    // 判斷是使用者或對方的訊息
                    if (data[m].opponent_id != 'bot'){
                        var name = eval('name_'+session_id);
                        var profile_picture = eval('picture_'+session_id);
                    }else{
                        var name = eval('name_'+'bot');
                        var profile_picture = eval('picture_'+'bot');
                    };
                    var msg = data[m].msg;
                    var pic = data[m].pic;
...
function submit_function() {
    ...
    if (typeof(inputText) == "undefined" || inputText.length < 1) {}
        ...
        socket.send(JSON.stringify(msg));
//        if (checkPictureUrl(inputText)){
//            updateLog(user_name, user_picture, null, inputText)
//        }else{
//            updateLog(user_name, user_picture, inputText, null);
//        };
//        view_check();
//        messageHistory += 1;
    };
...
function checkPictureUrl(url){
    re = new RegExp("http[s]?://[a-zA-Z0-9./_]+\.(jpg|png|bmp|gif)", "gi")
    if (re.exec(url)){
        return true
    }
};
function getProfile(id){
    $.get(http_domain_name + "/get_profile/"+id, function(profile_response) {
        var profile = JSON.parse(profile_response)
        var name = profile['name']
        eval('name_'+id+'='+'"'+name+'"');
        var picture = profile['picture']
        eval('picture_'+id+'='+'"'+picture+'"');
    });
};
function createChat(opponent_id){
    $(document).queue("chatqueue_"+opponent_id,function(){
        getProfile(opponent_id);
        $(document).dequeue("chatqueue_"+opponent_id);
    });
    $(document).queue("chatqueue_"+opponent_id,function(){
